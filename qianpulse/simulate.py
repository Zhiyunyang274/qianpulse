...
单次穿越必须"看不出桥"：悬架、路面、冲击与噪声把桥梁响应淹没，
这样融合收敛的戏剧性才来自算法，而不是数据本身太干净。
"""
...
def simulate_crossing(fs=100, duration=8.0, bridge_freq=7.8, vehicle_freq=None,
                      noise_std=1.0, rng=None):
    rng = np.random.default_rng() if rng is None else rng
    n = max(32, int(fs * duration))
    t = np.arange(n) / fs
    # 车辆固有频率部分落入分析频带，制造随机的"伪桥梁峰"
    vehicle_freq = float(vehicle_freq if vehicle_freq is not None else rng.uniform(1.5, 4.5))
    bridge_amp = rng.uniform(0.35, 0.75)
    vehicle_amp = rng.uniform(0.7, 1.6)
    ...
    roughness_freq = rng.uniform(9, 18)
    roughness = rng.uniform(0.35, 0.8) * np.sin(2 * np.pi * roughness_freq * t + rng.uniform(0, 2 * np.pi))
...
def simulate_batch(n_crossings=40, fs=100, duration=8.0, bridge_freq=7.8, seed=42,
    ...
    return [simulate_crossing(fs=fs, duration=duration, bridge_freq=bridge_freq,
                              vehicle_freq=rng.uniform(1.5, 4.5),
                              noise_std=rng.uniform(*noise_range), rng=rng)
